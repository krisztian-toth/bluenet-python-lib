from BluenetLib.lib.protocol.BlePackets import (
    ControlPacket,
    FactoryResetPacket, keepAliveStatePacket
)
...
class ControlPacketsGenerator:
    ...
    def getSetSchedulePacket(data):
        return ControlPacket(ControlType.SCHEDULE_ENTRY).loadByteArray(
            data).getPacket()

    @staticmethod
    def getScheduleRemovePacket(timerIndex):
        return ControlPacket(ControlType.SCHEDULE_REMOVE).loadUInt8(
            timerIndex).getPacket()
    ...
    def getSwitchStatePacket(switchState):
        ...
        return ControlPacket(ControlType.SWITCH).loadUInt8(
            convertedSwitchState).getPacket()
    ...
    def getPwmSwitchPacket(switchState):
        ...
        return ControlPacket(ControlType.PWM).loadUInt8(
            convertedSwitchState).getPacket()
    ...
    def getKeepAliveStatePacket(changeState, switchState, timeout):
        ...
        return keepAliveStatePacket(actionState, convertedSwitchState,
                                    timeout).getPacket()
    ...
    def getResetErrorPacket(errorMask):
        return ControlPacket(ControlType.RESET_ERRORS).loadUInt32(
            errorMask).getPacket()
    ...
    def getAllowDimmingPacket(allow):
        ...
        return ControlPacket(ControlType.ALLOW_DIMMING).loadUInt8(
            allowByte).getPacket()
    ...
    def getLockSwitchPacket(lock):
        ...
        return ControlPacket(ControlType.LOCK_SWITCH).loadUInt8(
            lockByte).getPacket()
